...
import { getCardAriaLabel } from '../utils/cardUtils';
...
const Card: React.FC<CardProps> = ({ card, animationDelay = 0 }) => {
    ...
    const ariaLabel = getCardAriaLabel(card);
                ...
                <div className="card-back" aria-label="Face down card">
                    <div className="w-full h-full bg-red-800 rounded-lg border-2 border-white flex items-center justify-center shadow-lg">
                        <div className="w-11 h-[70px] sm:w-12 sm:h-20 rounded-md border-2 border-gray-400 flex items-center justify-center bg-red-700">
                            <div className="text-2xl sm:text-3xl text-white font-bold opacity-50" aria-hidden="true">?</div>
                        </div>
                    </div>
                </div>

                {/* Front of the card (visible when flipped) */}
                <div className="card-front" aria-label={ariaLabel}>
                    <div className="w-full h-full bg-white rounded-lg p-1 sm:p-2 flex flex-col justify-between shadow-xl border border-gray-200" aria-hidden="true">
